<!DOCTYPE HTML>
<?php session_start(); include "includes/dbn.inc.php";include "includes/Buttonfunction.php";$per_page = 16;?>
				...
				<div class="col-xs-8 text-right menu-1">
					<ul>
						<?php if(isset($_SESSION['u_id']))
									echo '<li><a href="dashboard.php?login=success">Dashboard</a></li>';
								else
						...
						<form style="margin-top:-6px;" class="navbar-form navbar-right" role="search">
							...
							<button type="link" id="search-but" href="upload.php?param=search_param" class="btn btn-primary"><span id="gly-ser"class="glyphicon glyphicon-search"></span></button>
						</form>
						</li>
						...
						if (isset($_SESSION['u_id'])) {
							...
							if($_SESSION['type'] == 'teacher')
								$r = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM teachers WHERE user_id =".$_SESSION['u_id'].""));
							else
					...
					<div style="margin-top:1em" class="row row-mt-15em">
						...
						<?php
								if(!empty($parameters)){
									...
									$query="SELECT `n_id`,`author`,`n_name`,`course`,`stat`,`likes`,`date`,`file_loc`,`type` FROM `notes` WHERE `n_name` LIKE '%".$parameters."%' OR `course` LIKE '%".$parameters."%' AND `type`=0 AND `invalid` = 0 LIMIT $start,$per_page";
									$query_run=mysqli_query($conn,$query);//Running the query.
									$num_of_res=mysqli_num_rows($query_run);//Number of results.
									$query_for_count="SELECT `n_id`,`author`,`n_name`,`course`,`stat`,`likes`,`date`,`file_loc`,`type` FROM `notes` WHERE `n_name` LIKE '%".$parameters."%' OR `course` LIKE '%".$parameters."%' AND `type`=0 AND `invalid` = 0";
									...
									if($num_of_res!=0){?>
										...
										if($num_of_res>0){
												...
												?></h2>
		<div style="height:800px;margin-left:-64px;" class="tabovflow">
		<div style="height:760px;" class="mintabflow">
		<div id="tabcont" style="border-radius:0px;margin-left:0px;"class="jumbotronpers">
		<table class="uptab"><thead>
		<tr id="headerTab">
					<td>Note name</td>
					<td>Course name</td>
					<td>Author</td>
				...
				<tr>
					...
					<td><?php DownloadButton($loc,$_SESSION['u_uid'],$note_name);?></td>
										...
										<?php
												...
												<!--Page scroll-->
													<div id="upage" class="pages">
																<?php
												
																for((int)$i = 1;$i <= $pages;$i++){
																	?>
																<a href="upload.php?page=<?php echo $i;?>&search_param=<?php echo $parameters?>"><button style="border-radius:0px;height:38px;fon" class="btn btn-primary"><?php echo $i;?></button></a>
																<?php
																		}
																	?>
														</div>		
		...
		<div id="profile" class="jumbotronpers">
			...
			<div class="row">
				<div class="col-md-12 col-md-offset-0 text-left">
					...
					<div style="margin-top:10em" class="row row-mt-15em">
						<div style="margin-top:-6em;" class="col-md-4 mt-text animate-box" data-animate-effect="fadeInUp">
							<div id="quotes">
							
							<?php
								$Q_id = rand( 1 , 4 );
								$query = "SELECT quote FROM quotes WHERE id=".$Q_id."";
							...
							<form id="upload-form" class="col-md-12" action="upload.php" method="POST" enctype="multipart/form-data">
										<div style="border-radius:4px;padding-left:20px;padding-right:60px;width:400px;" class="jumbotron">
										<h2 style="color:black">Note/Literature Name</h2> <input id="nn" class="form-control" style="background-color:white" placeholder="Note/Literature Name" type="text" name="n_name" maxlength = "21" required /><br><br>
										<h2 style="color:black">Course/Literature Type</h2> <input id="cn" class="form-control" style="background-color:white" placeholder="Course/Literature Type" type="text" name="c_name" maxlength = "21"required ><br><br>
										<h2 style="color:black">Type</h2>
										<select id="tp" class="form-control" stye="color:white" name="nltype">
											<option value="l"><h4 style="color:white;">Literature</h4></option>
											<option value="n"><h4>Note</h4></option>
											<option value="p"><h4>Project Paper</h4></option>
											</select><br><br>
										<h2 style="color:black">Description(For lit./Project)</h2> <textarea id="ds" class="form-control" style="background-color:white" placeholder="Description" type="text" name="desc" rows="6" cols="15" maxlength = "180" ></textarea><br><br>	
										<input id="fileup" type="file" name="fileup" required><br><br>
									<button id="Upbut" class="btn btn-primary" type="submit" ><span class="glyphicon glyphicon-open"></span> Upload</button>
