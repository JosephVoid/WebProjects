...
<html>
			...
			if(isset($_GET['search_param'])){
				...
				<div class="page-inner">
					...
					<ul>
						...
						?>
						<li><a href="lit.php">Litrature</a></li>
						...
						<form style="margin-top:-6px;" class="navbar-form navbar-right" role="search">
							<div class="form-group">
								<input style="height: 30px;width: 115px;background:white;" type="text" name="search_param" class="form-control" placeholder="Search Projects">
							</div>
							<button type="link" id="search-but" href="projects.php?param=search_param" class="btn btn-primary"><span id="gly-ser"class="glyphicon glyphicon-search"></span></button>
						</form>
						</li>
	...
	<header id="gtco-header" class="gtco-cover" role="banner" style="background-image: url(images/back_im.jpg)" >
			...
			<div class="row">
				<div class="col-md-12 col-md-offset-0 text-left">
					<?php if(isset($_SESSION['u_id']))
						...
						$e = '13em';	
				?>
					<div style="margin-top:<?phpecho $e;?>;" class="row row-mt-15em">
		...
		<div style="height:852px;" class="mintabflow">										
			...
			while($query_row=mysqli_fetch_assoc($query_run))
				{
					...
					if(empty($query_row['description']))
		...
		?>
				...
				<tr>
					...
					<td>
						<a href="#/" data-toggle="collapse" data-target = "#desc<?php echo $note_id;?>" type="button">View Description</a>
							<div class="collapse" id="desc<?php echo $note_id;?>">
								<div style="padding:0px;text-align:left;font-size:16px;" class="well">
		...
		<!--Page scroll-->
						<div class="pages">
								...
								for((int)$i = 1;$i <= $pages;$i++){
									?>
								<a href="projects.php?page=<?php echo $i;?>&search_param=<?php echo $parameters?>"><button style="border-radius:5px;height:38px;" class="btn btn-primary"><?php echo $i;?></button></a>
								<?php
										}
									?>
						</div>		
			...
			<div class="row copyright">
				<div class="col-md-12">
					<p class="pull-left">
						...
						<small class="block">Designed by <a href="http://hireyab.cf/" target="_blank">Yeabsera G. Bune & Yoseph Tenaw</small> </a>
					</p>
					<p class="pull-right">
	...
	<div class="modal fade" id="profview">
		<div class="modal-dialog">
			<div id="md_oth" class="modal-content">
				<div style="height:100%;width:100%" class="modal-body">
					<!--Name & School-->
			...
			<div class="row">
						...
						<div  class="col-md-7 mt-text animate-box" data-animate-effect="fadeInUp">
							<div><h2>Ordered By</h2>
								...
								<select name="order_type" onchange = "location = this.value;">
									<option> </option>
									<option value="projects.php?order_type=l">Likes</option>
									<option value="projects.php?order_type=r">Recent</option>
								</select>
							</div>	
				...
				<table><thead>
					...
					$start = ($page - 1) * $per_page;
											...
											$queryml="SELECT * FROM `notes` WHERE `type` = '2' AND `invalid` = 0 ORDER BY ".$order." DESC LIMIT $start,$per_page";
											$query_for_count = "SELECT * FROM `notes` WHERE `type` = '2' AND `invalid` = 0 ORDER BY ".$order." DESC";
											$no_of_rows = mysqli_num_rows(mysqli_query($conn,$query_for_count));
											$pages = ceil($no_of_rows/$per_page);
											...
											while ( $query_rowsml = mysqli_fetch_assoc($query_runml)){
												...
												if(empty($query_rowsml['description']))
													$description = "No description";
												else
													$description = $query_rowsml['description'];
						...
						<div class="pages">
								...
								for((int)$i = 1;$i <= $pages;$i++){
									?>
								<a href="projects.php?page=<?php echo $i;?>"><button style="border-radius:0px;height:38px;fon" class="btn btn-primary"><?php echo $i;?></button></a>
								<?php
										}
									?>
						</div>		
