					...
					<ul>
						...
						<form style="margin-top:-6px;" class="navbar-form navbar-right" role="search">
							<div class="form-group">
								<input style="height: 30px;width: 115px;background:white;" type="text" name="search_param" class="form-control" placeholder="Search Litrature">
							</div>
							<button type="link" id="search-but" href="lit.php?param=search_param" class="btn btn-primary"><span id="gly-ser"class="glyphicon glyphicon-search"></span></button>
						</form>
						</li>
					</ul>
				</div>
								...
								if(!empty($parameters)){
									...
									if($num_of_res!=0){?>
										...
										if($num_of_res>0){
												...
												?></h2>
		<div style="height:898px;" class="tabovflow">
		<div style="height:860px;" class="mintabflow">										
		...
		<!--Page scroll-->
						<div class="pages">
								...
								for((int)$i = 1;$i <= $pages;$i++){
									?>
								<a href="lit.php?page=<?php echo $i;?>&search_param=<?php echo $parameters?>"><button style="height:38px;" class="btn btn-primary"><?php echo $i;?></button></a>
								<?php
										}
									?>
						</div>		
			...
			<div class="row">
						...
						<div  class="col-md-7 mt-text animate-box" data-animate-effect="fadeInUp">
							<div><h2>Ordered By</h2>
								...
								<select name="order_type" onchange = "location = this.value;">
									<option> </option>
									<option value="lit.php?order_type=l">Likes</option>
									<option value="lit.php?order_type=r">Recent</option>
								</select>
							</div>	
				...
				<thead><tr id="headerTab">
					...
					$start = ($page - 1) * $per_page;
											...
											$queryml="SELECT * FROM `notes` WHERE `type` = '1' ORDER BY ".$order." DESC LIMIT $start,$per_page";
											$query_for_count="SELECT * FROM `notes` WHERE `type` = '1' ORDER BY ".$order." DESC";
											$no_of_rows = mysqli_num_rows(mysqli_query($conn,$query_for_count));
											$pages = ceil($no_of_rows/$per_page);
						...
						<div class="pages">
								...
								for((int)$i = 1;$i <= $pages;$i++){
									?>
								<a href="lit.php?page=<?php echo $i;?>"><button style="border-radius:0px;height:38px;fon" class="btn btn-primary"><?php echo $i;?></button></a>
								<?php
										}
									?>
						</div>		
